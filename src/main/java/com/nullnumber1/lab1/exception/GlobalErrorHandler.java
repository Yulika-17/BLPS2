...
public class GlobalErrorHandler {
    ...
    @ResponseStatus(value = HttpStatus.BAD_REQUEST)
    public Error handlePaymentIsNotProcessedException(PaymentIsNotProcessedException ex, WebRequest request) {
        Error errorDTO = new Error(
                Code.BAD_REQUEST, new Date(), ex.getMessage(), request.getDescription(false));
        return errorDTO;
    }
    ...
    @ResponseStatus(value = HttpStatus.BAD_REQUEST)
    public Error handleDocumentGenerationException(DocumentGenerationException ex, WebRequest request) {
        Error errorDTO = new Error(
                Code.DOCUMENT_ERROR, new Date(), ex.getMessage(), request.getDescription(false));
        return errorDTO;
    }
