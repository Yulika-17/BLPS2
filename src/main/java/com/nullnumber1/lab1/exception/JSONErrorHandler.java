...
public class JSONErrorHandler {
  ...
  public Error handleUnrecognizedPropertyException(
      UnrecognizedPropertyException ex, WebRequest request) {
    Error error =
        new Error(
            Code.JSON_ERROR,
            new Date(),
            "Неидентифицированное поле в JSON.",
  ...
  public Error handleJsonException(Exception ex, WebRequest request) {
    Error error =
        new Error(Code.JSON_ERROR, new Date(), ex.getMessage(), request.getDescription(false));
    return error;
  }
}
